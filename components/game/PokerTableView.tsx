...
export default function PokerTableView({ gameState }: PokerTableViewProps) {
  ...
  return (
    <div className="table-rotate-wrapper sm:contents">
      <motion.div
      ...
      >
        ...
        {players.map((player, index) => {
          ...
          return (
            <div
              key={`bet-${player.id}`}
              className="absolute z-[15] bet-content"
              style={{
                left: `${betX}%`,
                top: `${betY}%`,
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none center-content">
        ...
        {players.map((player, index) => (
          <div
            key={player.id}
            className="absolute z-20 seat-content"
            style={{
